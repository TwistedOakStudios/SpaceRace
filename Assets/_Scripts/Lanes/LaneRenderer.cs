...
public class LaneRenderer : MonoBehaviour {
    ...
    public void AddTiles(int blue, int lastBlue, int red, int lastRed, int distance) {
    ...
    GameObject MakeTile(Tile type, float x, float y) {
        ...
        var go = Instantiate(prefab, new Vector3(x, -y), Quaternion.identity) as GameObject;
